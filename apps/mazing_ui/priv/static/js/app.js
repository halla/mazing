...
require.register("web/static/js/agent.js", function(exports, require, module) {
"use strict";
...
var Agent = {
  init: function init(socket, element) {
    socket.connect();
    var channel = socket.channel("agent:1", {});
    ...
    channel.on("agent_info", function (resp) {
      element.innerHTML = resp.html;
    });

    var agent_selector = document.getElementById("object-selector");
    agent_selector.addEventListener("change", function (e) {
      channel.push("set_active_agent", { agent: agent_selector.value });
    });

    document.getElementById('move-up').addEventListener('click', function (e) {
      channel.push("move", { direction: "up" });
    });
    document.getElementById('move-down').addEventListener('click', function (e) {
      channel.push("move", { direction: "down" });
    });
    document.getElementById('move-left').addEventListener('click', function (e) {
      channel.push("move", { direction: "left" });
    });
    document.getElementById('move-right').addEventListener('click', function (e) {
...
if (window.location.pathname == "/") {
  var maze = document.getElementById("maze-area");
  _maze2.default.init(_socket2.default, maze);
}
...
var agent = document.getElementById("agent-area");
_agent2.default.init(_socket2.default, agent);
});

;require.register("web/static/js/maze.js", function(exports, require, module) {
"use strict";
...
var Maze = {
  init: function init(socket, element) {
    var channel = socket.channel("maze:1", {});
    channel.join().receive("ok", function (resp) {
      console.log("Joined successfully", resp);
    }).receive("error", function (resp) {
      console.log("Unable to join", resp);
    ...
    var btn = document.getElementById("maze-me");
    btn.addEventListener("click", function (e) {
      var generator_selector = document.getElementById("generator-selector");
      channel.push("maze-me", { generator: generator_selector.value });
    });

    var bindCellListener = function bindCellListener() {
    ...
    socket.connect();
    channel.on("new_maze", function (resp) {
      element.innerHTML = resp.html;
      bindCellListener();
    });
    bindCellListener();
